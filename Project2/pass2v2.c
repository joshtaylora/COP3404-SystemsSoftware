...
typedef struct AssemblyProgram {
    AssemblyLine **programLines;
} AssemblyProgram;

// initialize with the number of lines in the program
AssemblyProgram AP_create(int programLines) {
    // allocate memory for the program
    AssemblyProgram *assemblyProgram = malloc( sizeof( AssemblyProgram ) * 1 );
...
AssemblyLine *AL_alloc(char *lineT, char *opT, char *opnd, int loc)
{
    ...
    strcpy(line->lineType, lineT);
    strcpy(line->optab, opT);
    strcpy(line->operand, opnd);
    line->location = loc;
...
int main(int argc, char *argv[]) {
    ...
    // create the assembly program type that stores the lines
